...
// and this impairs maximal segment recognition
	...
	const double size = 5.0; // size of displayed normals
...
// more curved zones are yellow till red.
		  ...
		  it != itE; ++it )
    {
		...
		if (processedPoints.find(closestPointToCurrent.getPoint()) != processedPoints.end())
			continue;
		
		processedPoints.insert(closestPointToCurrent.getPoint());
		
		if (processedPoints.size() == vPoints.size())
			break;
...
//		viewer << CustomColors3D(CURVE3D_COLOR, CURVE3D_COLOR)<< closestPointToCurrent.getPoint();
		...
		DGtal::functors::Point2DEmbedderIn3D<DGtal::Z3i::Domain> embedderVCM(domain3Dyup, *it, normal, radius * 2);
		ImageAdapterExtractor extractedImageVCM(volume, domainImage2D, embedderVCM, idV);
		Z2i::Point centerOfMass = SliceUtils::centerOfMass(extractedImageVCM);
